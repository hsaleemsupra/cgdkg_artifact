...
/// Computes the hash of a struct using an hash function that can be modelled as
/// a random oracle. Returns n elements in the scalar field of curve BLS12_381.
///
/// A distinct `domain` should be used for each purpose of the random oracle.
pub fn random_oracle_to_scalars(domain: &str, data: &dyn UniqueHash, n: usize) -> Vec<BIG> {
    let hash = random_oracle(domain, data);
    let rng = &mut RAND_ChaCha20::new(hash);
    let mut scalars = Vec::new();

    for _i in 0..n{
        scalars.push(rand_scalar_bls12381(rng));
    }
    return scalars;
}
